...
function gkScoped(scope){const all=[];if(typeof GK_HI_TOPICS!=='undefined'&&Array.isArray(GK_HI_TOPICS))GK_HI_TOPICS.forEach(topic=>{if(scope.length===0||scope.includes(Number(topic.id)))topic.questions.forEach(q=>all.push(norm({q:q[0],o:q[1],a:q[2]},'GK + Reasoning',{topic:topic.title})))});if(typeof REASONING_HI!=='undefined'&&Array.isArray(REASONING_HI))REASONING_HI.forEach(q=>all.push(norm({q:q[0],o:q[1],a:q[2]},'GK + Reasoning',{topic:'Reasoning'})));return all.filter(q=>q&&q.question&&q.answer>=0);}
function scopedPool(subjectKey,scope){if(subjectKey==='english')return englishScoped(scope);if(subjectKey==='gk')return gkScoped(scope);const pool=poolFor(subjectKey);return (scope.length===0?pool:pool.filter(q=>chapterMatch(q.chapterId,scope))).filter(q=>q&&q.question&&q.answer>=0);}
function exactSyllabusSummary(examNumber){const s=exactSyllabus(examNumber),range=k=>s[k].length?`Ch ${s[k].join(', ')}`:'All';return{science:range('science'),maths:range('maths'),english:range('english'),hindi:s.hindi.length?s.hindi.join(' • '):'All published topics',gk:s.gk.length?`Topics ${s.gk.join(', ')} + Reasoning`:'All GK topics + Reasoning',socialScience:range('socialScience')};}
function validateSyllabus(){Object.keys(WEEKLY_SYLLABUS).forEach(k=>{const s=WEEKLY_SYLLABUS[k];SUBJECT_KEYS.forEach(key=>{if(!Array.isArray(s[key]))throw new Error(`T${k} ${key} syllabus must be an array.`);if(new Set(s[key]).size!==s[key].length)throw new Error(`T${k} ${key} syllabus contains duplicates.`);});});return true;}
window.validateWeeklySyllabus=validateSyllabus;
validateSyllabus();
window.buildScopedWeeklyExam=function(exam){const s=exactSyllabus(exam.n),all=[];SUBJECT_KEYS.forEach(key=>{const p=scopedPool(key,s[key]);if(p.length<10)throw new Error(`T${exam.n} ${key} syllabus provides only ${p.length} usable questions; need 10.`);all.push(...seededShuffle(p,`${window.WEEKLY_EXAM_UTILS.getExamId(exam)}:${key}`).slice(0,10));});return seededShuffle(all,`${window.WEEKLY_EXAM_UTILS.getExamId(exam)}:paper`);};
...
function exactSyllabusDisplayKey(){const st=window.WEEKLY_EXAM_UTILS.getState();return st.exam?`T${st.exam.n}`:'';}
function decoratePlanner(){const body=document.getElementById('plannerBody');if(!body||!body.rows.length)return;[...body.rows].forEach((row,i)=>{const key=exactSyllabusDisplayKey();const target=window.WEEKLY_EXAM_SYLLABUS[i+1];if(!target||!row.cells[4])return;const summary=exactSyllabusSummary(i+1);row.cells[4].textContent=`Science: ${summary.science} | Maths: ${summary.maths} | English: ${summary.english} | Hindi: ${summary.hindi} | GK: ${summary.gk} | SST: ${summary.socialScience}`;});}
function decorateWeeklyBanner(){const banner=document.getElementById('weeklyExamBanner');if(!banner)return;const st=window.WEEKLY_EXAM_UTILS.getState();if(!st.exam)return;const box=banner.querySelector('.weekly-syllabus');if(!box)return;const summary=exactSyllabusSummary(st.exam.n),cards=[['🔬 Science',summary.science],['➗ Mathematics',summary.maths],['🇬🇧 English',summary.english],['🪔 Hindi',summary.hindi],['🧠 GK + Reasoning',summary.gk],['🌍 Social Science',summary.socialScience]];const key=`T${st.exam.n}`;if(box.dataset.syllabusKey===key)return;box.dataset.syllabusKey=key;box.innerHTML=cards.map(x=>`<div class="ws"><b>${x[0]}</b>${x[1]}</div>`).join('');}
