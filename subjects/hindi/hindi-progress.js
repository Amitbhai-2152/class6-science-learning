(function(){if(!window.XPSystem){document.write('<script src="../../js/xp-system.js?v=4"><\/script>')}document.write('<script src="../../js/xp-unify-bridge-v2.js?v=2"><\/script>')})();
(function(){
'use strict';
...
function maybeCelebrate(chapter,score,total,pct){ensureChapterCompletion();const id=Math.max(1,Number(new URLSearchParams(location.search).get('chapter'))||1),run=()=>window.ChapterCompletion?.show({subject:'hindi',chapterId:id,title:chapter||`अध्याय ${id}`,score,total,pct});if(window.ChapterCompletion)run();else setTimeout(run,60)}
...
function record(kind,score,total,topic){const x=read(),t=Math.max(1,Number(total)||1),s=Math.max(0,Number(score)||0),pct=Math.round(s/t*100);active(x);x.attempts++;x.best=Math.max(x.best,pct);x.xp+=Math.max(5,Math.round(s*2+5));if(topic)x.topics[topic]=Math.max(Number(x.topics[topic]||0),pct);x.history.unshift({kind,score:s,total:t,pct,topic:topic||'',time:Date.now()});x.history=x.history.slice(0,30);badges(x);const saved=save(x);if(kind==='chapter'&&pct>=75)maybeCelebrate(topic,s,t,pct);return saved}
