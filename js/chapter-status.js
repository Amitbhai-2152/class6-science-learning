...
function installStyle(){
  ...
  s.textContent='.chapter-status-badge{display:block;width:max-content;max-width:100%;margin-bottom:8px;padding:5px 8px;border-radius:999px;background:#f3f4f6;color:#667085;border:1px solid #e5e7eb;font-size:10px;font-weight:900;line-height:1.25}.chapter-status-badge.done{background:#ecfdf3;color:#15803d;border-color:#bbf7d0}.dark .chapter-status-badge{background:#2b3138;color:#d0d5dd;border-color:#444b55}.dark .chapter-status-badge.done{background:#123523;color:#86efac;border-color:#276749}';
...
function getStatus(subject,id){
  try{
    ...
    return{completed:true,at:Number(localStorage.getItem(DATE_PREFIX+subject+':'+id)||0),score:localStorage.getItem(scoreKey(subject,id))||''};
  }catch(_){return null}
}
...
function chapterIdFromHref(href){
  const m=href.match(/(?:chapter\.html\?chapter=|topic(?:-hi)?\.html\?topic=)([^&#]+)|(?:^|[?&])chapter=([^&#]+)/i);
...
function decorate(root=document){
  ...
  root.querySelectorAll('a[href]').forEach(a=>{
    ...
    const id=chapterIdFromHref(a.getAttribute('href')||'');if(!id)return;
    const status=getStatus(subject,id);
    a.dataset.chapterStatusApplied='1';
    const badge=document.createElement('span');
    badge.className='chapter-status-badge'+(status?' done':'');
    badge.textContent=status?formatDate(status.at):'📘 अभी पूरा नहीं किया';
    badge.title=status?.score?`Cleared score: ${status.score}`:'यह अध्याय अभी clear नहीं हुआ है';
    const go=a.querySelector('.go');
    if(go&&status){go.textContent='✓ अध्याय पहले पूरा किया गया • दोबारा खोलें →';go.style.color='var(--ok,#15803d)'}
