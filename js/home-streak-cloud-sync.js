(() => {
  ...
  function setPending(force=false){if(!window.Class6CloudSync?.configured?.())return false;if(cloudResolved&&!force)return false;document.documentElement.dataset.streakCloudPending='1';['streakMini','homeStreak'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent='…';});return true}
  ...
  function activityFromCloud(state){const s=state&&typeof state==='object'?state:{};const ss=s.streakState&&typeof s.streakState==='object'?s.streakState:{};return normalize({activeDays:[...(ss.activeDays||[]),...(s.activeDays||[])]})}
  ...
  async function sync(){
    ...
    syncPromise=(async()=>{
      try{
        ...
        if(current&&window.XPSystem?.save)window.XPSystem.save(Object.assign({},current,{activeDays:merged.activeDays,streakState:merged}));
        // Streak sync writes only streakState. XP score/subject fields remain owned by XP sync.
        const result=await window.Class6CloudSync.save({streakState:merged},1);
        if(result?.synced===true){
          window.dispatchEvent(new CustomEvent('class6:streak-cloud-synced',{detail:{userId:user.id,streak:merged.streak,lastActive:merged.lastActive,activeDays:merged.activeDays,synced:true}}));
          markCloudResolved();window.HomeStreak?.refresh?.();
        }else{
          throw new Error(String(result?.reason||'streak-cloud-sync-failed'));
        }
