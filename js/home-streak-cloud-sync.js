(() => {
  ...
  function activityFromCloud(state){const s=state&&typeof state==='object'?state:{};const ss=s.streakState&&typeof s.streakState==='object'?s.streakState:{};return normalize({activeDays:[...(s.activeDays||[]),...(ss.activeDays||[])]})}
  ...
  async function sync(){
    ...
    syncPromise=(async()=>{
      try{
        ...
        // IMPORTANT: only write streak fields. Sending the whole local XP snapshot here
        // could overwrite newer XP that another browser has already saved to the cloud.
        const result=await window.Class6CloudSync.save({activeDays:merged.activeDays,streakState:merged},1);
