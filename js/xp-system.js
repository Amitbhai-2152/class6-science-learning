...
function seedLegacy(){if(state.legacySeeded)return;const legacy={science:legacyValue('class6ScienceProgressV9'),maths:legacyValue('mathsExamHistory'),english:legacyValue('class6EnglishProgressV1'),hindi:legacyValue('class6HindiProgressV2'),gk:legacyValue('class6GKProgressV1'),social:legacyValue('socialScienceProgressV3'),revision:legacyValue('class6RevisionProgressV1')};state.total=0;Object.keys(state.subjects).forEach(s=>{const v=Math.max(0,Math.round(legacy[s]||0));state.subjects[s]=v;state.total+=v});state.legacySeeded=true;state.daily={date:today(),earned:0};save()}
seedLegacy();
...
window.XPSystem={KEY,DAILY_CAP,read:()=>state,save,seedLegacy,award,score,attemptXP,level,snapshot};
})();
