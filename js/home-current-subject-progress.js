...
const ROUTES={maths:'subjects/maths/index.html',english:'subjects/english/index.html',hindi:'subjects/hindi/index.html',gk:'subjects/gk/index.html',social:'subjects/social-science/index.html'};
...
function currentSubject(){
 ...
 if(params.get('view')==='science'){try{localStorage.setItem(KEY,'science')}catch(_){}return'science'}
 try{const saved=localStorage.getItem(KEY);if(SUBJECTS[saved])return saved}catch(_){}
 return'science';
}
function setText(id,text){const el=document.getElementById(id);if(el)el.textContent=text}
function setWidth(id,pct){const el=document.getElementById(id);if(el)el.style.width=clamp(pct)+'%'}
function wireContinue(id){
 const card=document.querySelector('.home-continue .home-card:first-child');
 const old=card?.querySelector('.home-btn.primary');
 if(!old)return;
 if(old.dataset.subjectRoute===id)return;
 const button=old.cloneNode(true);
 button.removeAttribute('onclick');
 button.dataset.subjectRoute=id;
 if(id==='science'){
   button.onclick=()=>{
     const s=window.HomeContinueFix?.getState?.();
     if(s&&typeof window.openChapter==='function')window.openChapter(s.chapter,s.part);else if(typeof window.openChapter==='function')window.openChapter(Math.max(1,Number(window.currentChapter)||1));
   };
 }else{
   button.onclick=()=>{window.location.href=ROUTES[id]||ROUTES.maths};
 }
...
function sync(){
 ...
 if(id!=='science'){
   setText('continueTitle',meta.label);
   setText('continuePercent',pct+'%');
   setText('continueMeta',meta.label+' • learning progress');
   setWidth('continueBar',pct);
   wireContinue(id);
 }
}
