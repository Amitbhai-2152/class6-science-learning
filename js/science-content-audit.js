...
function inspect(ch){
  ...
  const qs=Array.isArray(ch?.challenge)?ch.challenge:(Array.isArray(ch?.questions)?ch.questions:(Array.isArray(ch?.quiz)?ch.quiz:[]));
  const issues=[];
  if(sections.length<MIN_SECTIONS)issues.push(`less than ${MIN_SECTIONS} lessons`);
  if(qs.length<MIN_QUESTIONS)issues.push(`less than ${MIN_QUESTIONS} challenge questions`);
