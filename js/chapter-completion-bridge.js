...
function wrapOnce(obj,name,handler){if(!obj||installed.has(obj)||typeof obj[name]!=='function')return false;const original=obj[name].bind(obj);obj[name]=function(...args){const result=original(...args);try{handler(result,args)}catch(_){}return result};return true}
function install(){
  ...
  if(window.SocialScienceProgress&&!installed.has(window.SocialScienceProgress)){
    const p=window.SocialScienceProgress;
    let wrapped=false;
    if(typeof p.practice==='function'){
      const original=p.practice.bind(p);p.practice=function(id,score,total){const result=original(id,score,total);const pct=pctOf(score,total);if(pct>=75)showSocialScience(id,score,total,pct);return result};wrapped=true;
    }
    if(typeof p.test==='function'){
      const original=p.test.bind(p);p.test=function(id,score,total,type){const result=original(id,score,total,type);const pct=pctOf(score,total);if(pct>=75)showSocialScience(id,score,total,pct);return result};wrapped=true;
    }
...
function ensureLoader(){
  ...
  if(!document.querySelector('link[data-universal-chapter-completion-style]')){
    const l=document.createElement('link');l.rel='stylesheet';l.href=new URL('../css/chapter-completion.css?v=2',src).href;l.setAttribute('data-universal-chapter-completion-style','true');document.head?.appendChild(l)
  }
