...
function loadGlobalScript(file,attr,version){
  try{
    const scriptUrl=SCRIPT_SRC||document.querySelector('script[src*="theme-toggle.js"]')?.src||'';
    if(!scriptUrl||document.querySelector(`script[${attr}],script[src*="${file}"]`))return;
    const src=new URL(`./${file}?v=${version}`,scriptUrl).href;
    const s=document.createElement('script');s.src=src;s.async=false;s.setAttribute(attr,'true');(document.head||document.documentElement).appendChild(s);
  }catch(_){}
}
...
loadGlobalScript('home-notifications.js','data-home-notifications',1);
})();
